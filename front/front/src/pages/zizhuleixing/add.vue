	...
	export default {
		data() {
			return {
				...
				rules: {
					zizhuleixing: [
						{ required: true, message: '资助类型不能为空', trigger: 'blur' },
					],
				},
				centerType: false,
			};
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`zizhuleixing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`zizhuleixing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
