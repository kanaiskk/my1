	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zizhushenqing?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">发放状态</div>
						<div class="text "  >{{detail.fafangzhuangtai}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">证明材料</div>
						<el-button class="uploadBtn" @click="download(detail.zhengmingcailiao )">点击下载</el-button>
					</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zizhushenqing','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zizhushenqing','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('zizhushenqing','资助记录')" 
							@click="onAcross('zizhujilu','','','fafangzhuangtai','已发放','已发放,未发放'.split(',')[0])" type="warning">
							资助记录
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('zizhushenqing','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`zizhushenqing`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/zizhushenqing', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/zizhushenqingAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此资助申请？') .then(_ => {
					this.$http.post('zizhushenqing/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
