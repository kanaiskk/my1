<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="发放状态" prop="fafangzhuangtai">
				<el-select v-model="ruleForm.fafangzhuangtai" placeholder="请选择发放状态" :disabled="true  ||ro.fafangzhuangtai" >
					<el-option
			...
			<el-form-item class="add-item" label="证明材料" prop="zhengmingcailiao">
				<file-upload
					tip="点击上传证明材料"
					action="file/upload"
					:limit="1"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`zizhushenqing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`zizhushenqing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			zhengmingcailiaoUploadChange(fileUrls) {
				this.ruleForm.zhengmingcailiao = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
