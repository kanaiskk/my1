	...
	export default {
		data() {
			return {
				...
				rules: {
					zizhurenzhanghao: [
						{ required: true, message: '资助人账号不能为空', trigger: 'blur' },
					],
					mima: [
						{ required: true, message: '密码不能为空', trigger: 'blur' },
					],
					zizhurenxingming: [
						{ required: true, message: '资助人姓名不能为空', trigger: 'blur' },
					],
					xingbie: [
					],
					touxiang: [
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`zizhuren/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`zizhuren/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
