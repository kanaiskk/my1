	...
	<div class="main-containers">
		<div class="body-containers">
			<div class="top-container">
				<!-- info -->
				<div class="top_title">
					<span @click="goMenu('/index/home')">贫困儿童资助系统</span>
				</div>
	...
	methods: {
		...
		async getSession() {
			await this.$http.get(`${localStorage.getItem('UserTableName')}/session`, {emulateJSON: true}).then(async res => {
				if (res.data.code == 0) {
					if(localStorage.getItem('UserTableName')== 'ertong') {
						localStorage.setItem('username', res.data.data.ertongzhanghao);
					}
					if(localStorage.getItem('UserTableName')== 'zizhuren') {
						localStorage.setItem('username', res.data.data.zizhurenzhanghao);
					}
		...
		.body-containers {
			...
			.top-container {
				...
				.top_title {
					...
					box-shadow: rgb(205, 150, 41) 0px 4px 4px 0px;
					padding: 0 10px;
					top: 0;
					left: 90px;
					background: #F8CC74;
