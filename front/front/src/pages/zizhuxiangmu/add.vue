	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="资助类型" prop="zizhuleixing">
				<el-select v-model="ruleForm.zizhuleixing" placeholder="请选择资助类型" :disabled=" false  ||ro.zizhuleixing" >
					<el-option
			...
			<el-form-item class="add-item" label="资助描述" prop="zizhumiaoshu">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="资助描述"
			...
			<el-form-item class="add-item" label="资助详情" prop="zizhuxiangqing">
				<editor 
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get('option/zizhuleixing/zizhuleixing', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			// 多级联动参数
			info() {
				this.$http.get(`zizhuxiangmu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`zizhuxiangmu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
