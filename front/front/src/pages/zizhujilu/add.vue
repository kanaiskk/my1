	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="资助人账号" prop="zizhurenzhanghao">
				<el-select  @change="zizhurenzhanghaoChange" v-model="ruleForm.zizhurenzhanghao" placeholder="请选择资助人账号">
					<el-option
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get('option/zizhuren/zizhurenzhanghao', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						this.zizhurenzhanghaoOptions = res.data.data;
					}
				});
			...
			zizhurenzhanghaoChange () {
				this.$http.get('follow/zizhuren/zizhurenzhanghao?columnValue=' + this.ruleForm.zizhurenzhanghao, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						if(res.data.data.zizhurenxingming){
							this.ruleForm.zizhurenxingming = res.data.data.zizhurenxingming
						}
					}
				});
			},
			...
			// 多级联动参数
			info() {
				this.$http.get(`zizhujilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`zizhujilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
									duration: 1500,
									onClose: () => {
										if(this.isBackAuth('zizhujilu','支付')&&this.type=='cross'){
											this.$confirm('是否跳转支付？') .then(_ => {
												let jumpParams = {
													id: res.data.data,
													centerType: 1
												}
												this.$router.push({path: '/index/zizhujiluDetail', query: jumpParams});
											}).catch(_ => {
												this.$router.go(-1);
											});
										}else {
