<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="资助金额" prop="zizhujine">
				<el-input-number v-model="ruleForm.zizhujine" placeholder="资助金额" :disabled=" false ||ro.zizhujine"></el-input-number>
			</el-form-item>
			<el-form-item class="add-item" label="资助时间" prop="zizhushijian">
				<el-date-picker
					:disabled=" false  ||ro.zizhushijian"
			...
			<el-form-item class="add-item" label="资助内容" prop="zizhuneirong">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="资助内容"
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						...
						if((json.zizhurenzhanghao!=''&&json.zizhurenzhanghao) || json.zizhurenzhanghao==0){
							this.ruleForm.zizhurenzhanghao = json.zizhurenzhanghao;
							this.ro.zizhurenzhanghao = true;
						}
						if((json.zizhurenxingming!=''&&json.zizhurenxingming) || json.zizhurenxingming==0){
			...
			// 多级联动参数
			info() {
				this.$http.get(`xuanzezizhu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`xuanzezizhu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
									duration: 1500,
									onClose: () => {
										if(this.isBackAuth('xuanzezizhu','支付')&&this.type=='cross'){
											this.$confirm('是否跳转支付？') .then(_ => {
												let jumpParams = {
													id: res.data.data,
													centerType: 1
												}
												this.$router.push({path: '/index/xuanzezizhuDetail', query: jumpParams});
											}).catch(_ => {
												this.$router.go(-1);
											});
										}else {
