<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('zizhuxiangmu','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{zizhuxiangmuCount}}</div>
					<div class="name">资助项目总数</div>
				</div>
			</div>
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('zizhushenqing','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{zizhushenqingCount}}</div>
					<div class="name">资助申请总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<el-dialog :visible.sync="newsVisible" title="政策公告" :append-to-body="true" width="60%">
		...
		<!-- 3 -->
		<div class="type3">
			<div id="zizhuxiangmuChart1" class="echarts1 animate__animated" v-if="isAuth('zizhuxiangmu','首页统计')"></div>
			<div id="zizhuxiangmuChart2" class="echarts2 animate__animated" v-if="isAuth('zizhuxiangmu','首页统计')"></div>
			<div id="zizhushenqingChart1" class="echarts3 animate__animated" v-if="isAuth('zizhushenqing','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getzizhuxiangmuCount();
		if(this.isAuth('zizhuxiangmu','首页统计')){
			this.zizhuxiangmuChat1();
		}
		if(this.isAuth('zizhuxiangmu','首页统计')){
			this.zizhuxiangmuChat2();
		}
		this.getzizhushenqingCount();
		if(this.isAuth('zizhushenqing','首页统计')){
			this.zizhushenqingChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'zizhuxiangmuChart1',css:'animate__zoomIn'},
				{id:'zizhuxiangmuChart2',css:'animate__zoomIn'},
				{id:'zizhushenqingChart1',css:'animate__zoomIn'},
			]
		...
		getzizhuxiangmuCount() {
			this.$http({
				url: `zizhuxiangmu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.zizhuxiangmuCount = data.data
				}
			})
		},
// 1234 折
		zizhuxiangmuChat1() {
			this.$nextTick(()=>{
				...
				var zizhuxiangmuChart1 = echarts.init(document.getElementById("zizhuxiangmuChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						zizhuxiangmuChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							zizhuxiangmuChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		zizhuxiangmuChat2() {
			this.$nextTick(()=>{
				...
				var zizhuxiangmuChart2 = echarts.init(document.getElementById("zizhuxiangmuChart2"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						zizhuxiangmuChart2.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							zizhuxiangmuChart2.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		getzizhushenqingCount() {
			this.$http({
				url: `zizhushenqing/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.zizhushenqingCount = data.data
				}
			})
		},
// 1234 横
		zizhushenqingChat1() {
			this.$nextTick(()=>{
				...
				var zizhushenqingChart1 = echarts.init(document.getElementById("zizhushenqingChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						zizhushenqingChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							zizhushenqingChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
