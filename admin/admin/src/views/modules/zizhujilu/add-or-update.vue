	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="申请金额" prop="zizhujine" >
					<el-input-number v-model="ruleForm.zizhujine" placeholder="申请金额" :disabled="ro.zizhujine"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="申请金额" prop="zizhujine" >
					<el-input v-model="ruleForm.zizhujine" placeholder="申请金额" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'"  label="儿童账号" prop="ertongzhanghao" >
					<el-input v-model="ruleForm.ertongzhanghao" placeholder="儿童账号" clearable  :readonly="ro.ertongzhanghao"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="儿童账号" prop="ertongzhanghao" >
				...
				<el-form-item v-else class="input" label="儿童姓名" prop="ertongxingming" >
					<el-input v-model="ruleForm.ertongxingming" placeholder="儿童姓名" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'" label="资助人账号" prop="zizhurenzhanghao" >
					<el-select :disabled="ro.zizhurenzhanghao" @change="zizhurenzhanghaoChange" v-model="ruleForm.zizhurenzhanghao" placeholder="请选择资助人账号">
						<el-option
							v-for="(item,index) in zizhurenzhanghaoOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item class="input" v-else-if="ruleForm.zizhurenzhanghao" label="资助人账号" prop="zizhurenzhanghao" >
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				this.$http({
					url: `option/zizhuren/zizhurenzhanghao`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.zizhurenzhanghaoOptions = data.data;
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			// 下二随
			zizhurenzhanghaoChange () {
				this.$http({
					url: `follow/zizhuren/zizhurenzhanghao?columnValue=`+ this.ruleForm.zizhurenzhanghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.zizhurenxingming){
							this.ruleForm.zizhurenxingming = data.data.zizhurenxingming
						}
					} else {
						this.$message.error(data.msg);
					}
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.fafangxiangqing = this.ruleForm.fafangxiangqing.replace(reg,'../../../helpPoorChildren/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `zizhujilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										duration: 1500,
										onClose: () => {
											if(this.isAuth('zizhujilu','支付')&&this.type=='cross') {
												this.$confirm('是否跳转支付？').then(_ => {
													this.parent.showFlag = true;
													this.parent.addOrUpdateFlag = false;
													this.parent.zizhujiluCrossAddOrUpdateFlag = false;
													this.$router.push('/zizhujilu')
												}).catch(_ => {
													this.parent.showFlag = true;
													this.parent.addOrUpdateFlag = false;
													this.parent.zizhujiluCrossAddOrUpdateFlag = false;
